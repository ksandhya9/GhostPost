...
- **Technical Tasks**:
    - [x] Create `INTENTS` configuration object with IDs, labels, and icons.
    - [x] Implement `IntentSelector` component with grid layout.
    - [x] Set `activeView` to 'guided' and initialize workflow state.
    - [x] Add `targetAudience` input field to the workflow.
...
- **Technical Tasks**:
    - [x] Implement large `textarea` with auto-focus.
    - [x] Add character minimum validation (e.g., 10 chars).
    - [x] Create `generateStructurePrompt` builder in backend.
...
- **Technical Tasks**:
    - [x] Create API endpoint `POST /api/enhance/guided/structure`.
    - [x] Implement hook selection cards with "Option X" badges.
    - [x] Store `selectedHook` and `selectedStructure` in state.
...
- **Technical Tasks**:
    - [x] Create API endpoint `POST /api/enhance/guided/post`.
    - [x] Implement `generatePostPrompt` with `HUMAN_GUIDELINES` and `AI_BAN_LIST`.
    - [x] Render the draft in a high-fidelity editor view.
...
- **Technical Tasks**:
    - [x] Implement `useEffect` hook to sync `intent`, `messyIdea`, and `step` to `localStorage`.
    - [x] Hydrate state on initial component mount.
...
- **Technical Tasks**:
    - [x] Implement `prevStep()` and `nextStep()` with bounds checking.
    - [x] Ensure "Back" button is visible and functional on all steps except Step 1.
...
- **Technical Tasks**:
    - [x] Create API endpoint `POST /api/enhance/guided/variations`.
    - [x] Implement `generateVariationsPrompt` (Short, Story, Authoritative, etc.).
    - [x] Render variations in a responsive grid.
